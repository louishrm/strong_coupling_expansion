...
TEST(FermionOperatorTest, AtomicStates) {
  ...
  FermionOperator<double> op = FermionOperator<double>(0); //c_dn
  EXPECT_EQ(op.get_orbital_index(), 0);
  EXPECT_EQ(op.get_action(), 0);

  op = FermionOperator<double>(3); //cdag_up
  EXPECT_EQ(op.get_orbital_index(), 1);
  EXPECT_EQ(op.get_action(), 1);

  FockState state = FockState(1);              //|down>
...
TEST(FermionOperatorTest, SparseMatrixAtomic) {
  FermionOperator<double> cdag_up = FermionOperator<double>((1 << 1) | 1);
...
TEST(FermionOperatorTest, SpinFlip) {
  ...
  FermionOperator<double> cdag_up = FermionOperator<double>(3);
