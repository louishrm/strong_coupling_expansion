...
namespace sc_expansion {
  ...
  template <int N_sites, typename T> struct Args {
    ...
    std::vector<FermionOperator<N_sites, T>> ops; // alternating c^dag, c, c^dag, c, ...

    Args(std::vector<double> taus_, std::vector<FermionOperator<N_sites, T>> ops_);

    int order;
    static constexpr int N_ORBITALS = 2 * N_sites;

    double permutation_sign; // sign from sorting operators by descending imaginary time
